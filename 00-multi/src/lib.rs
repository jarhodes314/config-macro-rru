...
// TODO - replace me with a suitable datastructure that deserializes to either a HashMap<String, T> or a T
#[derive(Deserialize, Serialize, PartialEq, Eq, Debug)]
#[serde(transparent)]
...
mod tests {
    ...
    fn multi_supports_multiple_named_cumulocity_configurations() {
        ...
        let dto: TEdgeConfigDto = toml::from_str(&format!("c8y.edge.url = \"{edge}\"\nc8y.cloud.url = \"{cloud}\"")).unwrap();
